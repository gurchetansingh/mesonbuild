...
class MachineHolder(InterpreterObject):
    ...
    @noKwargs
    def endian_method(self, args: T.List[T.Any], kwargs: T.Dict[str, T.Any]) -> str:
        return T.cast(str, self.holder.endian)
...
class ConvertInterpreter(interpreter.Interpreter):
    ...
    def __init__(
        ...
        **kwargs: T.Any,
        ...
        self.variables["host_machine"] = MachineHolder(
            self.build.environment.machines.host
        )
        self.variables["build_machine"] = MachineHolder(
            self.build.environment.machines.build
        )
        self.variables["target_machine"] = MachineHolder(
            self.build.environment.machines.target
        )
    ...
    def func_find_program(
        ...
        for prog_name in prog_names:
            ...
            script_path = os.path.join(
                self.environment.source_dir, self.subdir, prog_name
            )
    ...
    def run_command_impl(
        ...
        cmd_args: T.List[
            T.Union[Executable, programs.Program, Compiler, File, str]
        ] = []
        for arg in raw_args:
    ...
    def func_dependency(
        ...
        if dep_info:
            ...
            dep.is_found = True
            dep.version = (
                dep_info[0].get("version", "convert_instance")
                if dep_info
                else "convert_instance"
            )
        ...
        if required:
            raise MesonException(
                f"Dependency '{desired_dep_name}' not found and is required."
            )
