...
class AbstractCompiler(Compiler):
    ...
    def has_multi_arguments(
        ...
        fails = (
            self.conf.get("supported_arguments", {}).get("fails", {}).get("args", [])
        )
    ...
    def has_multi_link_arguments(
        ...
        fails = (
            self.conf.get("supported_link_arguments", {})
            .get("fails", {})
            .get("args", [])
        )
        return (all(a not in fails for a in args), True)
    ...
    ) -> T.Generator[CompileResult, None, None]:
...
class AbstractClangCCompiler(AbstractCompiler, ClangCCompiler):
    ...
    def __init__(
        self, conf: T.Dict[str, T.Any], choice: MachineChoice, env: Environment
    ):
        ...
        linker = None
        if linker_id:
            linker = GnuBFDDynamicLinker(
                [f"/dev/null/{linker_id}"], env, choice, "", []
            )
        super().__init__(
            conf,
            [],
...
class AbstractToolchainInfo:
    ...
    def host_supported(self) -> bool:
        return (
            self.toolchains[MachineChoice.HOST] == self.toolchains[MachineChoice.BUILD]
        )
...
class AbstractToolchain:
    ...
    def __init__(
        ...
        self.toolchain_info = AbstractToolchainInfo(
            build_machine_toolchain, host_machine_toolchain, toolchain_config
        )
        self.toolchains[MachineChoice.HOST] = toolchain_config.get(
            host_machine_toolchain
        )
        self.toolchains[MachineChoice.BUILD] = toolchain_config.get(
            build_machine_toolchain
        )

    def create_c_compiler(
        self, choice: MachineChoice
    ) -> T.Optional[AbstractClangCCompiler]:
        c_info = self.toolchains[choice].get("c")
        if not c_info:
            return None
        return AbstractClangCCompiler(c_info, choice, self.env)

    def create_cpp_compiler(
        self, choice: MachineChoice
    ) -> T.Optional[AbstractClangCppCompiler]:
        cpp_info = self.toolchains[choice].get("cpp")
        if not cpp_info:
            return None
        return AbstractClangCppCompiler(cpp_info, choice, self.env)

    def create_rust_compiler(
        self, choice: MachineChoice
    ) -> T.Optional[AbstractRustCompiler]:
