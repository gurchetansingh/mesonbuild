...
class SoongBackend(ConvertBackend):
    ...
    def add_flag_config(
        ...
        if target.language == "rust":
            target.module_type = "rust_defaults"
        else:
            target.module_type = "cc_defaults"
    ...
    def add_include_dir_config(
        ...
        target.module_type = "cc_library_headers"
        os_select = self.get_os_info(toolchain, MachineChoice.HOST)
        arch_select = self.get_arch_info(toolchain, MachineChoice.HOST)
    ...
    def add_build_target_config(
        ...
        if isinstance(instance, ConvertInstanceStaticLibrary):
            if target.rust_abi == RustABI.RUST:
                target.module_type = "rust_library"
            elif target.rust_abi == RustABI.C:
                target.module_type = "rust_ffi_static"
            else:
                target.module_type = "cc_library_static"
        else:
            if target.rust_abi == RustABI.C:
                target.module_type = "rust_ffi_shared"
            else:
                target.module_type = "cc_library_shared"
