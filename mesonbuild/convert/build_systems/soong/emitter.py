...
def _custom_target_emit(custom_target: ConvertCustomTarget) -> str:
    cmd = getattr(custom_target, "cmd", "")
...
def _flag_target_emit(flag: ConvertFlag) -> str:
    content_str = ""
    if getattr(flag, "project_native_args", False) and getattr(
        flag, "host_supported", False
    ):
        content_str += MULTILIB_TARGET_BLOCK
    return content_str


def _build_target_emit(build_target: ConvertBuildTarget) -> str:
    content_str = ""
    if getattr(build_target, "install", False):
...
class SoongEmitter(CommonEmitter):
    ...
    def emit(self, state_tracker: ConvertStateTracker) -> None:
        ...
        for subdir, targets in targets_by_subdir.items():
            ...
            with open(
                os.path.join(output_path, "Android.bp"), "w", encoding="utf-8"
            ) as f:
                f.write(content)
