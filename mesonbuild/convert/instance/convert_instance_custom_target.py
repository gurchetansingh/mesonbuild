...
def index_from_string(input_str: str) -> int:
    ...
    if input_str.startswith(valid_prefixes):
        index_as_str = (
            input_str.replace("@INPUT", "").replace("@OUTPUT", "").rstrip("@")
        )
        if index_as_str:
...
class ConvertInstanceCustomTarget:
    ...
    def get_python_target(self) -> T.Optional[ConvertInstancePythonTarget]:
        ...
        if self.project_config.dependencies.programs:
            tool_config = (
                ...
                or self.project_config.dependencies.programs.get("python")
                or {}
            )
            ...
            for key, val in custom_target.env.get_env({}).items():
                ...
                self.convert_instance_cmds.append(
                    ConvertCustomTargetCmdPart(
                        env_cmd, ConvertCustomTargetCmdPartType.STRING
                    )
                )
    ...
    def _parse_custom_target(self, custom_target: build.CustomTarget) -> None:
        ...
        for output in custom_target.outputs:
            ...
            if custom_target.capture:
                ...
                self.convert_instance_cmds.append(
                    ConvertCustomTargetCmdPart(
                        output_str, ConvertCustomTargetCmdPartType.OUTPUT
                    )
                )
            ...
            if self.project_config.dependencies.programs:
                ...
                if prog_config is None:
                    raise MesonException(
                        f"Type: {type(prog_name)} not present, exiting..."
                    )
    ...
    def _handle_string(
        self, command_string: str, custom_target: build.CustomTarget
    ) -> None:
        if command_string == "@INPUT@":
            ...
            processed_string = self._handle_normalized_string(
                normalized_string, custom_target
            )
            self.convert_instance_cmds.append(
                ConvertCustomTargetCmdPart(
                    processed_string, ConvertCustomTargetCmdPartType.STRING
                )
            )

    def _handle_normalized_string(
        self, normalized_str: str, custom_target: build.CustomTarget
    ) -> str:
        sanitized_parts = []
