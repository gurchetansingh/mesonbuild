...
def generate_fuchsia_toolchains(
    ...
    This function fetches both the toolchain and the SDK from CIPD.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        subprocess.run(["unzip", "-q", "-o", clang_zip, "-d", clang_extract_path], check=True)
        ...
        subprocess.run(["unzip", "-q", "-o", sdk_zip, "-d", sdk_extract_path], check=True)
        ...
        for config in configurations:
            ...
            sysroot_path = os.path.join(sdk_extract_path, "arch", config.sysroot_arch, "sysroot")
